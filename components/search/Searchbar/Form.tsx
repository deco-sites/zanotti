...
const script = (formId: string, name: string, slot: string) => {
  ...
  const suggestionsContainer = document.getElementById(slot) as
    | HTMLDivElement
    | null;
  const searchButton = document.getElementById("search") as
    | HTMLButtonElement
    | null;
  const closeButton = document.getElementById("close") as
    | HTMLButtonElement
    | null;
...
export default function Searchbar(
  { placeholder = "O que deseja?", loader }: SearchbarProps,
) {
  const slot = useId();
  return (
      ...
      <div class="w-full relative z-[3]">
        <div class="join flex gap-[20px] lg:gap-0 bg-white rounded-full pr-5 relative z-[2]">
          ...
          <button
            ...
            class="flex items-center justify-center bg-transparent border-none"
            aria-label="Search"
            tabIndex={-1}
          >
          ...
          <button
            id="close"
            class="hidden items-center justify-center bg-transparent border-none"
          >
        ...
        <div
          id={slot}
          class="w-full absolute left-0 top-[19px] bg-white px-5 z-[1] rounded-b-3xl shadow"
        />
