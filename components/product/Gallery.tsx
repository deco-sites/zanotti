...
export default function GallerySlider(props: Props) {
    ...
    return (<>
        ...
        <div class="relative w-full">
          <Slider class="carousel carousel-center gap-6 w-full">
            {images.map((img, index) => (<Slider.Item index={index} class="carousel-item w-full">
                <Image class="w-full object-contain bg-white rounded-[10px] lg:rounded-[20px]" sizes="(max-width: 640px) 100vw, 40vw" style={{ aspectRatio: ASPECT_RATIO }} src={img.url!} alt={img.alternateName} width={WIDTH} height={HEIGHT} preload={index === 0} loading={index === 0 ? "eager" : "lazy"}/>
              </Slider.Item>))}
          </Slider>
          ...
          <button class="btn btn-ghost inline-flex absolute top-2 right-2 bg-base-100 rounded-full" hx-on:click={useScript((id) => {
            // @ts-ignore .
      ...
      {images && (<ProductImageZoom id={zoomId} images={images}/>)}
    </>);
}
